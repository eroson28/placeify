...
app.get("/api/allTiles", async function (req, res) {
  try {
    ...
    res.json({
      tiles: enrichedTiles,
      editCooldownSeconds: app.locals.editCooldownSeconds,
      globalLastEditTimestamp: app.locals.globalLastEditTimestamp,
    });
    console.log(
      `Server /api/allTiles: Sending cooldown: ${app.locals.editCooldownSeconds}, timestamp: ${app.locals.globalLastEditTimestamp}`
    );
  } catch (error) {
